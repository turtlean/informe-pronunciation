...
% \textit{L1 norm:} $\| C \|_{1}$ defines a sharp-edge region

% When regularizing with an \textit{L2-norm-based} term, as the penalty is increased all
% parameters are reduced while still remaining non-zero. On the other side, when regularizing
% with \textit{Lasso} increasing the penalty will cause more and more of the parameters to be
% driven to zero. Thus